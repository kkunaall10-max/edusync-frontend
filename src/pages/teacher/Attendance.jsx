...
import { 
  ...
  ClipboardCheck, TrendingUp, Calendar, LogOut, Download, Megaphone, AlertCircle, Settings
} from 'lucide-react';
...
const Attendance = () => {
    ...
    const exportCSV = () => {
        ...
        const rows = students.map(s => [
            s.roll_number, s.full_name, attendance[s.id] || 'N/A', selectedDate
        ]);
        ...
        <div style={styles.pageWrapper}>
            ...
            <main style={styles.mainContent}>
                <div style={styles.headerGlass}>
                  ...
                  <div style={{display:'flex', gap:'12px', flexWrap: isMobile ? 'wrap' : 'nowrap', width: isMobile ? '100%' : 'auto'}}>
                    <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} style={{background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:'12px', padding:'10px 16px', color:'white', outline:'none', fontSize:'14px', cursor:'pointer', flex: 1}} />
                    <button 
                        onClick={exportCSV}
                        style={{background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:'12px', padding:'10px 16px', color:'white', cursor:'pointer', display:'flex', alignItems:'center', gap:'8px', fontSize:'14px', fontWeight:'700'}}
